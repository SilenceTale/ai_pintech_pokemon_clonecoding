...
public class FileUploadService {
    ...
    public List<FileInfo> upload(RequestUpload form) {
        ...
        for (MultipartFile file : files) {
            ...
            item.setGid(gid);
            item.setLocation(location); // 경로 설정
            item.setFileName(fileName); // 오리지널 파일 이름
            item.setExtension(extension);
            item.setContentType(file.getContentType()); // 파일 형식

            fileInfoRepository.saveAndFlush(item);

            // 1. 파일 업로드 정보를 DB에 기록 E
            ...
            // 2. 파일 업로드 처리 S
             ...
             // 디렉토리가 존재하지 않거나 파일로만 있는 경우 생성한다.
